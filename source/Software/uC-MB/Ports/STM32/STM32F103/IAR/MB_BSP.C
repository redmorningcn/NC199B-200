...
#define  BSP_COM4_REN               DEF_DISABLED
...
#define MBREN2_GPIO_PIN        GPIO_Pin_0             /* PA.00 */
...
#define MBREN4_GPIO_PIN        GPIO_Pin_12             /* PC.12 */
#define MBREN4_GPIO_PORT       GPIOC
#define MBREN4_GPIO_RCC        RCC_APB2Periph_GPIOC
#endif
/*
*********************************************************************************************************
*                                             LOCAL VARIABLES
...
void  MB_RTU_TmrInit (void)
{
    ...
    while( (clk_freq / prescaler/ MB_RTU_Freq) == 0 )           //redmorningcn 计算分频系数，不满足最小值要求
        prescaler /= 2;
    
    MB_Tmr_ReloadCnts   = clk_freq / prescaler/ MB_RTU_Freq - 1;
    ...
    TIM_TimeBaseStructure.TIM_Prescaler     = prescaler - 1;            // 分频36000       
