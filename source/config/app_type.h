   ...
   union ___u_err_mask_flag {
    ...
    union ___u_system_sts {
        struct  {
            ...
            u16                 OpenAir         : 1;        // D0：=1:打开气路；=0:无；
            u16                 CloseAir        : 1;        // D1：=1 关闭气路；=0:无；
            u16                 StartMeasure    : 1;        // D2：=1:开始测量；=0:无；
            u16                 EndMeasure      : 1;        // D3：=1:结束测量；=0:无；
            u16                 Store           : 1;        // D4：=1:数据保存；=0:无；
    ...
    u8                  MeasureSecond;          //1          ???·2aê?ê±??
    u8                  MeasureMin;             //1          ???·2aê?????ê±??
